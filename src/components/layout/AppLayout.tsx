...
const AppLayout = () => {
  ...
  useEffect(() => {
    if (user?.role === "waiter" && waiterRestrictedRoutes.includes(location.pathname)) {
      navigate("/"); // Redirect waiters to home or another allowed page
    }

    if (user?.role === "chef" && chefRestrictedRoutes.includes(location.pathname)) {
            ...
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <button className="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100">
                  <img src={getImageUrl(user?.image)} className="w-8 h-8 rounded-full" />
