      ...
      <DialogContent className="fixed inset-0 flex items-center justify-center p-4 bg-black bg-opacity-50 z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
          <div className="flex justify-end mb-4">
            <X className="h-6 w-6 cursor-pointer hover:bg-gray-100 rounded-full" onClick={onClose} />
          </div>
          <DialogHeader>
            <FadeInUp as="span">
              <DialogTitle className="text-lg font-semibold flex justify-between"><span>{item?.menu?.name}</span> <span>${item?.price * item?.quantity}</span></DialogTitle>
            </FadeInUp>
          </DialogHeader>
          ...
          <div className="flex my-4 justify-between gap-2">
            <ScaleIn className="w-full flex items-center">
              ...
              {['ready', 'cancelled'].includes(item.status) && <span>Qty: </span>} <span className="w-12 text-center text-lg">{item.quantity}</span>
            ...
            <ScaleIn className="w-full flex items-center justify-end">
              {['in-kitchen','ready', 'cancelled'].includes(item.status) && <OrderItemStatusBadge status={item.status} />}
              ...
              >
                <ChefHatIcon className="h-6 w-6" /> <span>Send to kitchen</span>
              </button>}
            </ScaleIn>
          </div>
